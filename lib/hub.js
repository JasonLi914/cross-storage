;(function(root) {
   ...
   * @param {instance} storage An instance of storage library
   */
  CrossStorageHub.init = function(permissions, storage) {
    ...
    // access is disabled
    try {
      if (!storage && !window.localStorage) available = false;
    } catch (e) {
      available = false;
    }
    ...
    CrossStorageHub._storage = storage || window.localStorage;
  ...
  CrossStorageHub._listener = function(message) {
    ...
    if (!method) {
      ...
      try {
        CrossStorageHub['_' + method](request.params).then(function(res) {
          CrossStorageHub._postMessage(origin, request, error, res);
        });
      } catch (err) {
        error = err.message;
        CrossStorageHub._postMessage(origin, request, error, result);
  ...
  CrossStorageHub._set = function(params) {
    return Promise.resolve(CrossStorageHub._storage.setItem(params.key, params.value));
  };
  ...
  CrossStorageHub._get = function(params) {
    ...
    for (i = 0; i < params.keys.length; i++) {
      result.push(storage.getItem(params.keys[i]));
    }
  ...
  CrossStorageHub._del = function(params) {
    ...
    for (var i = 0; i < params.keys.length; i++) {
      result.push(CrossStorageHub._storage.removeItem(params.keys[i]));
    }
  ...
  CrossStorageHub._clear = function() {
    return Promise.resolve(CrossStorageHub._storage.clear());
  };
  ...
  CrossStorageHub._getKeys = function(params) {
    ...
    keys = [];
    CrossStorageHub._storage.length().then(function (length) {
      for (i = 0; i < length; i++) {
